...
Formwerk is not an i18n library. It does not provide a way to translate the labels and messages. You should use a dedicated i18n library in tandem with Formwerk like [vue-i18n](https://vue-i18n.intlify.dev/).

## Locale

Formwerk exposes a `locale` configuration option that you can set with the `configure` function.
...
This directionality is used to determine the default `dir` property of Formwerk components. You can always override it with the `dir` prop on each composable or you can turn the auto-detection off with the `detectDirection` option.
...
Formwerk composables can be used in conjunction with [vue-i18n](https://vue-i18n.intlify.dev/). Usually it is a good idea to set the locale in the app's main file or entry point where you initialize vue-i18n.

Formwerk accepts a reactive locale value and will automatically update the directionality whenever the locale changes.
