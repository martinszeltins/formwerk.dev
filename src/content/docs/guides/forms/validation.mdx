...
You can disable HTML5 validation by setting `disableHtmlValidation` to `true`. This option exist as a prop on fields, forms, form groups or as a global configuration with the `configure` function.
...
As mentioned in the form guide. Standard schemas automatically offer type inference for the form's current values and the submit values. This is incredibly useful to to avoid having to cast and re-check values when submitting them just because TypeScript isn't aware of the runtime validation.
...
const onSubmit = handleSubmit((data) => {
  data.toJSON(); // { email: string; password: string }
});
```
...
As you have probably have noticed in the field guides, Formwerk unifies standard schema validation with the HTML constraint validation API.
