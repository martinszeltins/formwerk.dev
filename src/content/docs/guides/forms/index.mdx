...
You can set the touched state of a field manually with the `setFieldTouched` function.
...
You can alternatively use CSS with the `:user-invalid` pseudo-class to show errors only when the field is invalid and has been interacted with. More info on that in the [Styling guide](/guides/extras/styling/).
