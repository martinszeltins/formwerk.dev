...
import OptionPartialStyled from './_partials/_optionStyled.mdx';
import OptionGroupPartialStyled from './_partials/_optionGroupStyled.mdx';
import SelectPartialStyled from './_partials/_selectStyled.mdx';
...
We kept styling to a minimum, but we will take it up a notch or in the [styling section](#styling) below.
...
Because selects in Formwerk are a fully custom component, it doesn't support any HTML validation attributes. You can however, use Typed Schemas to validate the value of the slider.
...
`useSelect` supports Typed Schema validation through the `schema` prop.
...
Styling a select field can be tricky because of all the moving parts.

Aside from polishing the styles of the previous components. We also solved the popup positioning by using the [CSS anchor positioning](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_anchor_positioning), alternatively you can use [floating-ui](https://floating-ui.com/) or similar positioning libraries to position your popup.
...
const schema = defineSchema(z.string().min(1, 'Please select a country'));
...
<template>
  <SelectField
    ...
    :schema="schema"
    description="Delivery usually within 5-7 business days"
  >
    <OptionGroup
      v-for="group in groups"
      :key="group.label"
...
| schema      | `TypedSchema<Arrayable<TValue>>` (optional) | The schema for validating the select field value.      |
