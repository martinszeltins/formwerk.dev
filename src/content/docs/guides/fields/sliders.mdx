...
import SliderOriented from './_partials/_sliderOriented.mdx';
import SliderThumbStyled from './_partials/_sliderThumbStyled.mdx';
import SliderStyled from './_partials/_sliderStyled.mdx';
...
Because sliders in Formwerk are a fully custom component, it doesn't support any HTML validation attributes. You can however, use Typed Schemas to validate the value of the slider.

### Typed Schema

The `useSlider` composable accepts a `schema` prop that has a typed schema value.
...
const schema = defineSchema(z.number('Required').min(30).max(80));
</script>
...
Here is an example of how you can style the slider using plain CSS:
...
const schema = defineSchema(z.number().min(30).max(80));
</script>

<template>
  <Slider label="Opacity" :schema="schema" />
</template>
...
| ---------- | --------- | -------------------------------------------------------------------------------------------- |
| label      | `string`  | Optional label for the thumb, falls back to the slider's label.                              |
| modelValue | `number`  | The current value of the thumb.                                                              |
| disabled   | `boolean` | Whether the thumb is disabled or not. Doesn't affect validation or submission of the slider. |
...
| Name         | Type            | Description                                |
| ------------ | --------------- | ------------------------------------------ |
| thumbProps   | Complex Binding | Properties for the thumb element.          |
| currentValue | `number`        | Current value of the thumb.                |
| isDragging   | `boolean`       | Whether the thumb is being dragged or not. |
...
| name        | `string`              | Optional name for the slider   |
| orientation | `Orientation`         | Orientation of the slider      |
| dir         | `Direction`           | Direction of the slider        |
| modelValue  | `number \| number[]`  | Current value of the slider    |
| min         | `number`              | Minimum value of the slider    |
| max         | `number`              | Maximum value of the slider    |
| step        | `number`              | Step value for the slider      |
| disabled    | `boolean`             | Whether the slider is disabled |
| readonly    | `boolean`             | Whether the slider is readonly |
| schema      | `TypedSchema<number>` | Schema for the slider value    |
...
| errors            | `string[]`                              | The slider's error messages.                                         |
| fieldValue        | `number \| undefined`                   | The current value of the slider.                                     |
| groupProps        | Complex Binding                         | The slider group container.                                          |
| trackProps        | Complex Binding                         | The slider's track props.                                            |
| isDirty           | `boolean`                               | Whether the slider value has changed.                                |
| isTouched         | `boolean`                               | Whether the slider is touched.                                       |
| isValid           | `boolean`                               | Whether the slider has error messages.                               |
| labelProps        | `AriaLabelProps`                        | The label element props.                                             |
| setErrors         | `(msg: Arrayable<string>) => void`      | Sets the error messages for the slider and sets the `isValid` state. |
| setTouched        | `(touched: boolean) => void`            | Sets the `isTouched` state.                                          |
| setValue          | `(value: number) => void`               | Sets the slider's current value.                                     |
| useThumbMetadata  | `(thumbIndex: number) => ThumbMetadata` | Returns metadata for a slider thumb at the given index.              |

#### `useThumbMetadata` returns

Additionally, `useThumbMetadata` returns an object with the following properties:

| Name          | Type     | Description                                            |
| ------------- | -------- | ------------------------------------------------------ |
| value         | `number` | Current value of the specified thumb.                  |
| percent       | `number` | Current value percentage compared to the thumb's max.  |
| min           | `number` | The minimum possible value of the thumb.               |
| max           | `number` | The Maximum possible value of the thumb.               |
| sliderPercent | `number` | Current value percentage compared to the slider's max. |
