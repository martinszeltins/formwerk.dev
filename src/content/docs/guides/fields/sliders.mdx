...
- Keyboard support
  - <kbd>ArrowLeft</kbd> and <kbd>ArrowRight</kbd> keys increment and decrement
    the value respectively.
  - <kbd>Home</kbd> and <kbd>End</kbd> keys set the value to the minimum and
    maximum values respectively.
  - <kbd>PageUp</kbd> and <kbd>PageDown</kbd> keys increment and decrement the
    value by a large step respectively.
...
<template>
  <div
    v-bind="thumbProps"
    class="h-4 w-4 flex-shrink-0 rounded-full bg-gray-400 focus-visible:bg-blue-500"
  >
    <div
      v-if="isDragging"
      class="absolute -top-7 left-1/2 -translate-x-1/2 rounded bg-blue-400 px-1.5 py-1 text-xs font-medium text-white"
    >
      {{ currentValue }}
    </div>
  </div>
...
.slider {
  ...
  .track {
    @apply flex w-full items-center py-2;
    &::before {
      content: '';
